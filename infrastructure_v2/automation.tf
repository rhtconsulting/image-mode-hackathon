...
locals {
  ...
  quay_server_hostnames = [
    for name, server in local.flattened_servers :
    server.hostname
    if server.role == "quay"
  ]

  # Dynamically resolved from the servers created by this Terraform process.
  primary_quay_hostname = one(local.quay_server_hostnames)

  gitlab_server_hostnames = [
    for name, server in local.flattened_servers :
    server.hostname
    if server.role == "gitlab"
  ]

  # This automation expects exactly one server with role "gitlab".
  primary_gitlab_hostname = one(local.gitlab_server_hostnames)

  cop_webhook_workflow_name = (
    "[Image Mode CI/CD - AAP] Build sample RHEL 9 web image"
  )

  cop_webhook_gitlab_project = "image-mode/sample-rhel9-web"

  # Stable project/repository paths and default tag. The GitLab and Quay
  # hostnames are intentionally not set here. Ansible obtains them from the
  # inventory generated by this Terraform deployment.
  cop_sample_git_project_path = "image-mode/sample-rhel9-web.git"
  cop_sample_quay_namespace   = "image-mode"
  cop_sample_image_name       = "sample-rhel9"
  cop_sample_image_tag        = "v1"
...
resource "terraform_data" "bootstrap_lab" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      BRANCH="set-consistent-shell-for-aws"
...
resource "terraform_data" "deploy_cop_aap_pipeline" {
  ...
  triggers_replace = [
    ...
    local.primary_quay_hostname,
    local.cop_sample_git_project_path,
    local.cop_sample_quay_namespace,
    local.cop_sample_image_name,
    local.cop_sample_image_tag,
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      # Dynamic registry endpoint produced by this Terraform environment.
      QUAY_REGISTRY_HOST = local.primary_quay_hostname

      # Stable paths and tag used to construct workflow defaults. The service
      # endpoints themselves are read from the generated Ansible inventory.
      SAMPLE_GIT_PROJECT_PATH = local.cop_sample_git_project_path
      SAMPLE_QUAY_NAMESPACE   = local.cop_sample_quay_namespace
      SAMPLE_IMAGE_NAME       = local.cop_sample_image_name
      SAMPLE_IMAGE_TAG        = local.cop_sample_image_tag
    ...
    command = <<-EOT
      ...
      if [ -z "$QUAY_REGISTRY_HOST" ]; then
        echo "Terraform did not generate a Quay registry hostname." >&2
        exit 1
      fi

      case "$QUAY_REGISTRY_HOST" in
        http://*|https://*|*/*)
          echo "Invalid Quay registry hostname: $QUAY_REGISTRY_HOST" >&2
          echo "Expected a hostname without a scheme or repository path." >&2
          exit 1
          ;;
      esac

      if [ -z "$SAMPLE_GIT_PROJECT_PATH" ] ||
         [ -z "$SAMPLE_QUAY_NAMESPACE" ] ||
         [ -z "$SAMPLE_IMAGE_NAME" ] ||
         [ -z "$SAMPLE_IMAGE_TAG" ]; then
        echo "Sample GitLab/Quay workflow values were not generated." >&2
        exit 1
      fi
      ...
      jq -n \
        --arg custom_registry_url "$QUAY_REGISTRY_HOST" \
      ...
      if [ "$(jq -r '.custom_registry_url' "$COP_RUNTIME_VARS_FILE")" != \
           "$QUAY_REGISTRY_HOST" ] ||
         [ "$(jq -r '.quay_login_username' "$COP_RUNTIME_VARS_FILE")" != \
           "image-mode-builder" ] ||
      ...
      echo "Quay login registry: $QUAY_REGISTRY_HOST"
      echo "GitLab project path: $SAMPLE_GIT_PROJECT_PATH"
      echo "Quay namespace: $SAMPLE_QUAY_NAMESPACE"
...
############################################################
# Capture AAP Webhook And Configure GitLab
#
# AAP creates the webhook URL and key when the CoP workflow is configured with
# webhook_service: gitlab. This phase runs afterward, stores those generated
# values directly in AWS Secrets Manager, and reconciles the GitLab project
# hook without placing the webhook key in Terraform state.
############################################################
...
resource "terraform_data" "configure_cop_gitlab_webhook" {
  ...
  triggers_replace = [
    terraform_data.deploy_cop_aap_pipeline.id,
    local_file.ansible_inventory.content_sha256,
    local.primary_aap_url,
    local.primary_gitlab_hostname,
    local.cop_webhook_workflow_name,
    local.cop_webhook_gitlab_project,
    aws_secretsmanager_secret.cop_aap_webhook.arn,
    aws_secretsmanager_secret.generated[
      "aap/gateway_admin_password"
    ].arn
  ]
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      COP_WEBHOOK_SECRET_NAME = (
        aws_secretsmanager_secret.cop_aap_webhook.name
      )

      COP_WEBHOOK_WORKFLOW_NAME = local.cop_webhook_workflow_name
      GITLAB_HOSTNAME           = local.primary_gitlab_hostname
      GITLAB_PROJECT_PATH       = local.cop_webhook_gitlab_project
    }
    ...
    command = <<-EOT
      ...
      if ! command -v jq >/dev/null 2>&1; then
        echo "jq is required by the webhook configuration process." >&2
        exit 1
      fi
      ...
      if [ ! -f "$WEBHOOK_PLAYBOOK" ]; then
        echo "Webhook playbook not found: $WEBHOOK_PLAYBOOK" >&2
        exit 1
      fi

      export AAP2_CONTROLLER_PASSWORD="$(
        aws secretsmanager get-secret-value \
          --secret-id "$AAP_CONTROLLER_PASSWORD_SECRET" \
          --query SecretString \
          --output text
      )"

      if [ -z "$AAP2_CONTROLLER_PASSWORD" ]; then
        echo "AAP controller password is empty." >&2
        exit 1
      fi
      ...
      WORKFLOW_JSON="$(
        curl \
          ...
          --fail \
          --insecure \
          --user "admin:$AAP2_CONTROLLER_PASSWORD" \
          --get \
          --data-urlencode "name=$COP_WEBHOOK_WORKFLOW_NAME" \
          "$AAP2_CONTROLLER_URL/api/controller/v2/workflow_job_templates/"
      )"
      ...
      if [ "$WORKFLOW_COUNT" -ne 1 ]; then
        echo \
          "Expected exactly one AAP workflow named: $COP_WEBHOOK_WORKFLOW_NAME" \
          >&2
        exit 1
      fi
      ...
      WORKFLOW_DETAIL="$(
        curl \
          ...
          --insecure \
          --user "admin:$AAP2_CONTROLLER_PASSWORD" \
          "$AAP2_CONTROLLER_URL/api/controller/v2/workflow_job_templates/$WORKFLOW_ID/"
      )"

      WEBHOOK_SERVICE="$(
        printf '%s' "$WORKFLOW_DETAIL" |
          jq -er '.webhook_service'
      )"

      if [ "$WEBHOOK_SERVICE" != "gitlab" ]; then
        echo \
          "AAP workflow does not have webhook_service=gitlab: $COP_WEBHOOK_WORKFLOW_NAME" \
          >&2
        exit 1
      fi
      ...
      WEBHOOK_RECEIVER_PATH="$(
        printf '%s' "$WORKFLOW_DETAIL" |
          jq -er '.related.webhook_receiver'
      )"

      WEBHOOK_KEY_PATH="$(
        printf '%s' "$WORKFLOW_DETAIL" |
          jq -er '.related.webhook_key'
      )"

      case "$WEBHOOK_RECEIVER_PATH" in
        http://*|https://*)
          AAP_WEBHOOK_URL="$WEBHOOK_RECEIVER_PATH"
          ;;
        *)
          AAP_WEBHOOK_URL="$AAP2_CONTROLLER_URL$WEBHOOK_RECEIVER_PATH"
          ;;
      esac

      case "$WEBHOOK_KEY_PATH" in
        http://*|https://*)
          WEBHOOK_KEY_URL="$WEBHOOK_KEY_PATH"
          ;;
        *)
          WEBHOOK_KEY_URL="$AAP2_CONTROLLER_URL$WEBHOOK_KEY_PATH"
          ;;
      esac

      export AAP_WEBHOOK_KEY="$(
        curl \
          --silent \
          --show-error \
          --fail \
          ...
          --user "admin:$AAP2_CONTROLLER_PASSWORD" \
          "$WEBHOOK_KEY_URL" |
          jq -er '.webhook_key // .key'
      )"

      if [ -z "$AAP_WEBHOOK_URL" ] || [ -z "$AAP_WEBHOOK_KEY" ]; then
        echo "AAP did not return a usable webhook URL and key." >&2
        exit 1
      fi
      ...
      jq -n \
        --arg workflow_name "$COP_WEBHOOK_WORKFLOW_NAME" \
        --arg webhook_service "gitlab" \
        --arg webhook_url "$AAP_WEBHOOK_URL" \
        --arg webhook_key "$AAP_WEBHOOK_KEY" \
        --arg gitlab_project "$GITLAB_PROJECT_PATH" \
        '{
      ...
      aws secretsmanager put-secret-value \
        --secret-id "$COP_WEBHOOK_SECRET_NAME" \
        --secret-string "file://$WEBHOOK_SECRET_FILE" \
        --output text \
        --query VersionId >/dev/null

      unset AAP_WEBHOOK_KEY

      ansible-playbook \
        -i "$INVENTORY_FILE" \
        --extra-vars "cop_webhook_secret_name=$COP_WEBHOOK_SECRET_NAME" \
        --extra-vars "gitlab_project_path=$GITLAB_PROJECT_PATH" \
        "$WEBHOOK_PLAYBOOK"

      echo "CoP AAP webhook stored and GitLab project hook configured."
    EOT
  }
}
